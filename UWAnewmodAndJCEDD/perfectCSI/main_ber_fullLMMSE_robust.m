...
for n_mc = 1:N_mc
    ...
    for b = 1:length(sigma2_cand)
        ...
        data_rx = (H_OFDM'*H_OFDM+sigma2*eye_N) \ (H_OFDM'*r_rx_OFDM) ;
        bit_est = qamdemod(data_rx.',qam_mod,'OutputType','bit','UnitAveragePower',true) ;
        ber_OFDM(b) = ber_OFDM(b) ...
            + sum(sum(data_bit~=bit_est)) ;
        % SC
        data_rx = (H_SC'*H_SC+sigma2*eye_N) \ (H_SC'*r_rx_SC) ;
        bit_est = qamdemod(data_rx.',qam_mod,'OutputType','bit','UnitAveragePower',true) ;
        ber_SC(b) = ber_SC(b) ...
            + sum(sum(data_bit~=bit_est)) ;
        % ODDM
        data_rx = (H_ODDM'*H_ODDM+sigma2*eye_N) \ (H_ODDM'*r_rx_ODDM) ;
        bit_est = qamdemod(data_rx.',qam_mod,'OutputType','bit','UnitAveragePower',true) ;
        ber_ODDM(b) = ber_ODDM(b) ...
            + sum(sum(data_bit~=bit_est)) ;
        % proposed modulation
        data_rx = (H_propose'*H_propose+sigma2*eye_N) \ (H_propose'*r_rx_propose) ;
        bit_est = qamdemod(data_rx.',qam_mod,'OutputType','bit','UnitAveragePower',true) ;
