function [ber_iter,nmse_iter] = JCEDD_iter(r,F,...
    B_OMP_ini,B_OMP_base,sp,lg,ag,P_min,P_max,fc,T,N,Ng,roll_off,Q,qam_mod,N_iter,N_iter_in,sigma2,data_bit,H,rho)
...
eye_N = eye(N) ;
nmse_iter = zeros(N_iter*N_iter_in,1) ;
ber_iter = zeros(N_iter*N_iter_in,1) ;
sp_pre = sqrt(rho) * sp ;
sigma2_est = 5*sigma2 ;
d_min = 1e8 ;
for n_iter = 1:N_iter
    ...
    rmse = norm(H_est-H,'fro') / norm(H,'fro') ;
    nmse_iter((n_iter-1)*N_iter_in+1:...
        n_iter*N_iter_in) = rmse*rmse ;
    ...
    for n_iter_in = 1:N_iter_in
        ...
        ber_iter((n_iter-1)*N_iter_in+n_iter_in) = ...
